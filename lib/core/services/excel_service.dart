...
// Assume these are your actual imports
// import 'package:your_app/models/account.dart';
...
// import 'package:your_app/logic/session_manager.dart';
// import 'package:your_app/logic/auth_cubit.dart';
// import 'package:your_app/l10n/app_localizations.dart';
// import 'package:your_app/ui/widgets/master_password_dialog.dart';
...
class ExcelService {
  ...
  AppLocalizations get _l10n => AppLocalizations.of(NavigationService.navigatorKey.currentContext!)!;
  ...
  Future<({List<Account> accounts, Map<int, String> categoryMap})> _prepareExportData(int userId) async {
  ...
  Excel _buildExcelFile(({List<Account> accounts, Map<int, String> categoryMap}) data) {
  ...
  Future<void> _saveAndShareFile(List<int>? fileBytes) async {
    ...
    final filePath = '${directory.path}/PassKeeper_Backup_${DateTime.now().toIso8601String()}.xlsx';
  ...
  Future<String> importAccountsFromExcel() async {
    try {
      ...
      final result = await _processSheetRows(sheet, userId, profileTag, categoryMap);

      return _l10n.feedbackImportSuccess(result.success, result.skipped, result.failed);
    } catch (e) {
  ...
  Future<({int success, int skipped, int failed})> _processSheetRows(
      Sheet sheet, int userId, String profileTag, Map<String, int> categoryMap) async {
    ...
    for (var i = 1; i < sheet.maxRows; i++) {
      try {
        ...
        final newAccount = await _buildAccountFromRowData(rowData, userId, profileTag, categoryMap);
  ...
  ({String category, String serviceName, String username, String password, String? recovery, String? phone, String? customJson}) _parseRowData(List<Data?> row) {
    return (
    category: row.elementAtOrNull(0)?.value.toString() ?? '',
    serviceName: row.elementAtOrNull(1)?.value.toString() ?? '',
    username: row.elementAtOrNull(2)?.value.toString() ?? '',
    password: row.elementAtOrNull(3)?.value.toString() ?? '',
    recovery: row.elementAtOrNull(4)?.value.toString(),
    phone: row.elementAtOrNull(5)?.value.toString(),
    customJson: row.elementAtOrNull(6)?.value.toString(),
    );
  }

  /// Job: Transforms parsed row data into a final `Account` object.
  Future<Account> _buildAccountFromRowData(
      ({String category, String serviceName, String username, String password, String? recovery, String? phone, String? customJson}) rowData,
      int userId,
      String profileTag,
      Map<String, int> categoryMap,
      ) async {
    final categoryId = await _findOrCreateCategory(userId, rowData.category, profileTag, categoryMap);

    Map<String, String> customFields = {};
    if (rowData.customJson != null && rowData.customJson!.isNotEmpty) {
      try {
        customFields = Map<String, String>.from(jsonDecode(rowData.customJson!));
      } catch (e) {
  ...
  Future<Sheet?> _pickAndParseExcelSheet() async {
    ...
    if (sheet.maxRows == 0 || sheet.row(0).length < 6) {
      throw Exception("Invalid Excel format. Header row must contain at least 6 columns.");
    }
  ...
  Future<Map<String, int>> _getCategoryMap(int userId, String profileTag) async {
    final categories = await _databaseService.getCategories(userId, profileTag);
  ...
  Future<int> _findOrCreateCategory(int userId, String categoryName, String profileTag, Map<String, int> categoryMap) async {
